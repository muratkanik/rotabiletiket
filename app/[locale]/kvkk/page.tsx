...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
    ...
    return {
        title: locale === 'de' ? 'DSGVO / KVKK-Datenschutzhinweis | Rotabil Etiket' : 'KVKK Aydınlatma Metni | Rotabil Etiket',
        description: locale === 'de' ? 'Datenschutzhinweis gemäß dem türkischen Datenschutzgesetz KVKK.' : 'Kişisel Verilerin Korunması Kanunu kapsamında aydınlatma metni.'
    }
}
        ...
        <main className="min-h-screen bg-slate-50">
            ...
            <div className="bg-slate-900 py-16 text-white text-center">
                <h1 className="text-3xl md:text-4xl font-bold mb-4">
                    {locale === 'de' ? 'DSGVO / KVKK-Datenschutzhinweis' : 'KVKK Aydınlatma Metni'}
                </h1>
                <p className="text-slate-400">{locale === 'de' ? 'Schutz personenbezogener Daten' : 'Kişisel Verilerin Korunması'}</p>
            </div>
