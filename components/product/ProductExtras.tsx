...
export function ProductDurability({ durability }: { durability: any }) {
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 p-6 mb-8 shadow-sm">
            <h3 className="font-bold text-slate-900 mb-5">Dayanıklılık & Test Sonuçları</h3>
            {durability.heat && renderBar('Isı Dayanımı', <ThermometerSun className="w-4 h-4 text-orange-500" />, durability.heat, 'bg-orange-500')}
            {durability.chemical && renderBar('Kimyasal Dayanım', <Droplets className="w-4 h-4 text-purple-500" />, durability.chemical, 'bg-purple-500')}
            {durability.physical && renderBar('Fiziksel Yıpranma', <Shield className="w-4 h-4 text-blue-500" />, durability.physical, 'bg-blue-500')}
        </div>
    );
}

export function ProductDocuments({ documents }: { documents: any[] }) {
    if (!documents || documents.length === 0) return null;

    return (
        <div className="bg-white rounded-xl border border-slate-200 p-6 mb-8 shadow-sm">
            <h3 className="font-bold text-slate-900 mb-4">Teknik Dökümanlar & Sertifikalar</h3>
...
export function ProductFAQ({ faqs }: { faqs: any[] }) {
    if (!faqs || faqs.length === 0) return null;

    return (
        <div className="mb-8">
            <h3 className="font-bold text-slate-900 text-xl mb-6">Sık Sorulan Sorular</h3>
